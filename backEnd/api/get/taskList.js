...
const taskList = async (req,res,params,user) => {
   // 过滤条件  必须是当前用户的
    let sqlStr ='' 
    if(user.level === 10){
        sqlStr = `select * from task left join account on responsePerson=id
        where status=${params.status}
        `
    }else{
        sqlStr = `select * from task left join account on responsePerson=id
        where status=${params.status} and responsePerson='${user.id}'
        `
    }
    const tasks = await sql(sqlStr)
